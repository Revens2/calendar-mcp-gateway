"""Proxy vers le serveur MCP upstream (conteneur `calendar-mcp`, 127.0.0.1:3000).
...
- `tools/list` : la reponse upstream est filtree pour n'annoncer que les outils
  que le jeton a le droit d'utiliser ; `manage-accounts` (administration des
  comptes Google) n'est JAMAIS annonce ;
- `tools/call` : refuse localement (erreur JSON-RPC) si l'outil demande est en
  ecriture sans portee d'ecriture, est `manage-accounts` (meme avec la portee
  d'ecriture), ou n'est pas classe ; l'upstream n'est alors JAMAIS contacte ;

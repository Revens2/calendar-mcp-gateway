"""Assemblage de l'application ASGI de la passerelle Calendar.
...
def _config() -> tuple[str, str, int, str, str]:
    ...
    emetteur = _requise("CALENDAR_MCP_ISSUER")
    if not emetteur.startswith("https://"):
        raise RuntimeError("CALENDAR_MCP_ISSUER doit etre en HTTPS")
    upstream = _requise("CALENDAR_MCP_UPSTREAM")
    if not upstream.startswith("http://"):
        raise RuntimeError("CALENDAR_MCP_UPSTREAM doit etre en HTTP (boucle locale)")
    port = int(os.environ.get("CALENDAR_MCP_PORT", str(PORT_PAR_DEFAUT)))
    jeton = os.environ.get("CALENDAR_MCP_TOKEN", "")
    if jeton and len(jeton) < 32:
        raise RuntimeError("CALENDAR_MCP_TOKEN trop court : 32 caracteres minimum")
    return emetteur, upstream, port, jeton, os.environ.get("CALENDAR_MCP_OAUTH_DIR", "")
...
def construire_application(
    ...
    routes: list[Route] = [
        ...
        *create_protected_resource_routes(
            ...
            resource_name="Google Calendar MCP (passerelle)",
        ),
